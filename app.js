...
// ============================================================
const TITULOS = { mapa: "Cocheras", pagos: "Pagos", vehiculos: "Vehículos", alquileres: "Precios", aumentos: "Aumentos", mensajes: "Mensajes", espera: "Lista de espera", gastos: "Impuestos/Servicios", facturacion: "Facturación", mantenimiento: "Mantenimiento", recordatorios: "Recordatorios", notas: "Notas", backup: "Backup" };
...
// ============================================================
const CONDICIONES = {
  responsable_inscripto: { label: "Resp. Inscripto",  pill: "pill-ri"   },
  monotributo:           { label: "Monotributo",       pill: "pill-mono" },
  consumidor_final:      { label: "Cons. Final",       pill: "pill-cf"   },
  exento:                { label: "Exento",            pill: "pill-ex"   },
  no_responsable:        { label: "No Responsable",    pill: "pill-otro" },
  exterior:              { label: "Suj. del Exterior", pill: "pill-otro" }
};
...
function renderFacturacion() {
  ...
  lista.forEach(([id, p]) => {
    ...
    panel.innerHTML = `
      <div class="fact-panel-header">
        ...
        <div class="fact-panel-btns">
          ...
          <button class="btn-copiar-fact" title="Copiar datos para factura">
          ...
          ${wsp ? `<button class="btn-wsp fact-wsp-btn">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.149-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M11.99 2C6.477 2 2 6.477 2 12c0 1.89.525 3.66 1.438 5.168L2 22l4.985-1.31A9.954 9.954 0 0 0 11.99 22C17.522 22 22 17.523 22 12S17.522 2 11.99 2zm.01 18.181a8.17 8.17 0 0 1-4.165-1.138l-.299-.177-3.093.812.825-3.02-.194-.31A8.185 8.185 0 0 1 3.818 12C3.818 7.479 7.48 3.818 12 3.818c4.522 0 8.182 3.661 8.182 8.182 0 4.522-3.66 8.181-8.182 8.181z"/></svg>
            WhatsApp
          </button>` : ""}
        </div>
      ...
      <div class="fact-panel-body">
        ...
        <div class="fact-fields">
          <div class="fact-field">
            <span class="fact-field-label">Condición fiscal</span>
    ...
    // Copiar datos para factura
    panel.querySelector(".btn-copiar-fact").addEventListener("click", (e) => {
      e.stopPropagation();
      const cond = CONDICIONES[p.condicion] || { label: p.condicion || "—" };
      const lines = [
        `Nombre: ${p.nombre || "—"}`,
        p.razon  ? `Razón social: ${p.razon}` : null,
        `Condición fiscal: ${cond.label}`,
        p.cuit   ? `CUIT/CUIL: ${p.cuit}` : null,
        p.dni    ? `DNI: ${p.dni}` : null,
        ultimoPago ? `Período: ${mesLabel(ultimoPagoMes)}` : null,
        ultimoPago ? `Monto: ${formatMonto(ultimoPago.monto)}` : null,
        ultimoPago ? `Forma de pago: ${ultimoPago.metodo === "transferencia" ? "Transferencia" : "Efectivo"}` : null,
        v ? `Concepto: Alquiler cochera Nº ${String(v.cochera).padStart(2,"0")}` : null,
      ].filter(Boolean).join("\n");

      navigator.clipboard.writeText(lines).then(() => {
        showToast("Datos copiados al portapapeles ✓", "success");
      }).catch(() => {
        showToast("No se pudo copiar. Usá HTTPS.", "error");
      });
    });
    ...
    if (btnWsp && wsp) {
      btnWsp.addEventListener("click", (e) => {
        ...
        window.open(`https://wa.me/54${num}`, "whatsapp_tab");
      });
    }
...
document.getElementById("btn-imprimir-cocheras")?.addEventListener("click", async () => {
  ...
  wrap.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;
      ...
      <div style="font-size:16px;font-weight:700;color:#111">JPSoft | Garage — Cocheras</div>
    ...
    <div style="margin-top:16px;font-size:10px;color:#ccc;text-align:right">JPSoft | Garage</div>`;
