...
// ============================================================
const TITULOS = { mapa: "Cocheras", vehiculos: "Vehículos", alquileres: "Alquileres", pagos: "Pagos", espera: "Lista de espera", mantenimiento: "Mantenimiento", gastos: "Impuestos y Servicios", mensajes: "Mensajes", facturacion: "Facturación", aumentos: "Aumentos", recordatorios: "Recordatorios", backup: "Backup" };
...
function exportarJSON() {
  const datos = { vehiculos, pagos, espera: listaEspera, mantenimiento, gastos, facturacion, aumentos, recordatorios, config: { totalEspacios } };
  ...
  lista.forEach(([id, p]) => {
    ...
    card.innerHTML = `
      ...
      <div class="fact-info">
        ...
        ${p.notas ? `<div class="fact-razon">${p.notas}</div>` : ""}
      </div>
    ...
    `;
    card.addEventListener("click", () => abrirFactModal(id));
    list.appendChild(card);
  });
}
...
function abrirRecModal(id = null) {
  ...
  document.getElementById("rec-ultimo").value       = r.ultimo       || "";
  document.getElementById("rec-proximo").value      = r.proximo      || "";
...
async function guardarRec() {
  ...
  const ultimo  = document.getElementById("rec-ultimo").value;
  const proximo = document.getElementById("rec-proximo").value;
  if (!titulo)  { document.getElementById("rec-titulo").focus();  showToast("Ingresá un título", "error"); return; }
  if (!ultimo)  { document.getElementById("rec-ultimo").focus();  showToast("Ingresá el último control", "error"); return; }
  if (!proximo) { document.getElementById("rec-proximo").focus(); showToast("Ingresá el próximo control", "error"); return; }
