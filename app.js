...
// ============================================================
document.getElementById("btn-imprimir-cocheras")?.addEventListener("click", () => {
  const hoy = new Date().toLocaleDateString("es-AR", {
    day:"2-digit", month:"2-digit", year:"numeric", hour:"2-digit", minute:"2-digit"
  });

  // Crear contenedor de impresión
  let printDiv = document.getElementById("print-cocheras");
  if (!printDiv) {
    printDiv = document.createElement("div");
    printDiv.id = "print-cocheras";
  ...
  for (let i = 1; i <= totalEspacios; i++) {
    ...
    const v       = entrada ? entrada[1] : null;
    celdas += `
      <div class="print-cell ${libre ? "libre" : "ocupado"}">
        <div class="print-num">${String(i).padStart(2,"0")}</div>
        <div class="print-nombre">${libre ? "Libre" : (v.nombre || "").split(/[\s,]+/)[0]}</div>
        ${!libre && v ? `<div class="print-tipo">${v.modelo || TIPOS[v.tipo] || ""}</div>` : ""}
      </div>`;
  }
  ...
  printDiv.innerHTML = `
    <div class="print-header">
      <div>
        <div class="print-title">JPSoft | Garage — Estado de cocheras</div>
      </div>
      <div class="print-fecha">${hoy} · ${ocupadosCount} ocupadas · ${libresCount} libres</div>
    </div>
