...
let factEditId    = null;
let factEmitidas  = {};   // { factId: { fid, periodo, monto, cae, cae_vto, nro, fecha, pdf_link } }

// ============================================================
//  🧾 TUSFACTURAS — CREDENCIALES
//  Completá con tus datos de TusFacturasAPP
// ============================================================
const TF_USERTOKEN = "TU_USERTOKEN";   // ← reemplazá
const TF_APIKEY    = "TU_APIKEY";      // ← reemplazá
const TF_APITOKEN  = "TU_APITOKEN";    // ← reemplazá
const TF_PTO_VENTA = "0001";           // ← tu punto de venta en ARCA
...
function renderFacturacion() {
  ...
  lista.forEach(([id, p]) => {
    ...
    // Factura emitida en el período actual
    const hoyKey      = (() => { const h = new Date(); return `${h.getFullYear()}-${String(h.getMonth()+1).padStart(2,"0")}`; })();
    const factMesActual = Object.values(factEmitidas).find(f => f.fid === id && f.periodo === hoyKey);
    ...
    panel.innerHTML = `
      <div class="fact-panel-header">
        ...
        <div class="fact-panel-btns">
          ...
          ${factMesActual
            ? `<span class="fact-emitida-badge">✓ Factura emitida — CAE ${factMesActual.cae}</span>`
          ...
          ${factMesActual && p.mail ? `<button class="btn-mail-fact fact-mail-btn">
          ...
          ${factMesActual && wsp ? `<button class="btn-wsp-fact fact-wsp-btn">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.149-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M11.99 2C6.477 2 2 6.477 2 12c0 1.89.525 3.66 1.438 5.168L2 22l4.985-1.31A9.954 9.954 0 0 0 11.99 22C17.522 22 22 17.523 22 12S17.522 2 11.99 2zm.01 18.181a8.17 8.17 0 0 1-4.165-1.138l-.299-.177-3.093.812.825-3.02-.194-.31A8.185 8.185 0 0 1 3.818 12C3.818 7.479 7.48 3.818 12 3.818c4.522 0 8.182 3.661 8.182 8.182 0 4.522-3.66 8.181-8.182 8.181z"/></svg>
            Enviar por WhatsApp
          </button>` : ""}
        </div>
      ...
      <div class="fact-panel-body">
        ...
        <div class="fact-fields">
          <div class="fact-field">
            <span class="fact-field-label">Condición frente al IVA</span>
            <span class="fact-field-val">${cond.label}</span>
          </div>
          <div class="fact-field">
            <span class="fact-field-label">CUIT / CUIL</span>
            <span class="fact-field-val mono">${p.cuit || "—"}</span>
          </div>
          <div class="fact-field">
            <span class="fact-field-label">DNI</span>
            <span class="fact-field-val mono">${p.dni || "—"}</span>
          </div>
          <div class="fact-field">
            <span class="fact-field-label">Razón social</span>
            <span class="fact-field-val">${p.razon || "—"}</span>
          </div>
          ${v ? `<div class="fact-field" style="grid-column:1/-1">
            <span class="fact-field-label">Concepto</span>
            <span class="fact-field-val">Alquiler cochera Nº ${String(v.cochera).padStart(2,"0")}</span>
          </div>` : ""}
          <div class="fact-field" style="grid-column:1/-1">
            <span class="fact-field-label">Mail</span>
            <span class="fact-field-val">${p.mail || "—"}</span>
          </div>
          ${p.notas ? `<div class="fact-field" style="grid-column:1/-1">
            <span class="fact-field-label">Notas</span>
            <span class="fact-field-val">${p.notas}</span>
          </div>` : ""}
        </div>
        ${ultimoPago ? `
        <div class="fact-section-label" style="margin-top:12px">Último pago registrado</div>
        <div class="fact-fields">
          <div class="fact-field">
            <span class="fact-field-label">Período</span>
            <span class="fact-field-val">${mesLabel(ultimoPagoMes)}</span>
          </div>
          <div class="fact-field">
            <span class="fact-field-label">Monto</span>
            <span class="fact-field-val mono">${formatMonto(ultimoPago.monto)}</span>
            <span style="font-size:12px;color:var(--text2);margin-top:3px;font-family:'DM Mono',monospace;user-select:text;cursor:text">ARCA: ${Math.round(ultimoPago.monto)}</span>
          </div>
          <div class="fact-field">
            <span class="fact-field-label">Método</span>
            <span class="fact-field-val">${ultimoPago.metodo === "transferencia" ? "📲 Transferencia" : "💵 Efectivo"}</span>
          </div>
          <div class="fact-field">
            <span class="fact-field-label">Cobró</span>
        ...
        <div class="fact-historial">
          ${historial.map(f => `
            <div class="fact-hist-row">
              ...
              <span class="fact-hist-cae">CAE ${f.cae}</span>
              ${f.pdf_link ? `<a href="${f.pdf_link}" target="_blank" class="fact-hist-pdf">PDF</a>` : ""}
            </div>
          `).join("")}
        </div>`;
        })()}
    ...
    // Mail — Enviar factura (con CAE y PDF si ya fue emitida)
    ...
    if (btnMail && p.mail) {
      btnMail.addEventListener("click", (e) => {
        ...
        const pdfLinea = factMesActual?.pdf_link ? `\n\nDescargá tu factura aquí: ${factMesActual.pdf_link}` : "";
        const caeLinea = factMesActual?.cae ? `\nCAE: ${factMesActual.cae}` : "";
        const asunto   = encodeURIComponent(`Factura — Alquiler cochera Nº ${cochera} — ${periodo}`);
        const cuerpo   = encodeURIComponent(
          `Hola, ${nombre}!\n\nTe envío la factura correspondiente al alquiler de la cochera Nº ${cochera} del período ${periodo} por un monto de ${monto}.${caeLinea}${pdfLinea}\n\nSaludos!\n\n\n${adminNombre}.\n\n\nJPSoft | Cocheras`
        );
        const a = document.createElement("a");
    ...
    // WhatsApp — Enviar factura (con CAE y PDF si ya fue emitida)
...
// ============================================================
//  TUSFACTURAS — EMISIÓN DE FACTURA ELECTRÓNICA
// ============================================================

// Mapeo condición IVA → código TusFacturasAPP
const TF_CONDICION = {
  responsable_inscripto:        "RI",
  monotributo:                  "M",
  monotributista_social:        "M",
  monotributista_independiente: "M",
  consumidor_final:             "CF",
  sujeto_exento:                "EX",
  no_categorizado:              "NC",
  proveedor_exterior:           "E",
  cliente_exterior:             "E",
  iva_liberado:                 "CF",
  iva_no_alcanzado:             "CF",
};
...
async function emitirFactura(fid) {
  ...
  // Monto: del pago del mes actual o del precio base del vehículo
  const pagoMes = pagos[mesKey]?.[vid];
  const monto   = pagoMes?.monto || v.monto || 0;
  ...
  if (yaEmitida) {
    showToast(`Ya se emitió factura para ${mesLabel(mesKey)}`, "error");
    return;
  }

  // Credenciales incompletas
  if (TF_USERTOKEN === "TU_USERTOKEN") {
    showToast("Completá las credenciales de TusFacturasAPP en app.js", "error");
    return;
  }

  const fechaHoy    = hoy.toLocaleDateString("es-AR", { day:"2-digit", month:"2-digit", year:"numeric" }).replace(/\//g, "/");
  const primerDia   = `01/${String(hoy.getMonth()+1).padStart(2,"0")}/${hoy.getFullYear()}`;
  const ultimoDia   = new Date(hoy.getFullYear(), hoy.getMonth()+1, 0);
  const ultimoDiaTxt = `${String(ultimoDia.getDate()).padStart(2,"0")}/${String(hoy.getMonth()+1).padStart(2,"0")}/${hoy.getFullYear()}`;

  const condTF   = TF_CONDICION[p.condicion] || "CF";
  const docTipo  = p.cuit ? "CUIT" : "DNI";
  const docNro   = p.cuit || p.dni || "0";
  const razonSocial = p.razon || p.nombre || "Consumidor Final";
  ...
  const body = {
    ...
    comprobante: {
      ...
      tipo:                     "FACTURA C",
      operacion:                "V",
      punto_venta:              TF_PTO_VENTA,
      moneda:                   "PES",
      cotizacion:               1,
      periodo_facturado_desde:  primerDia,
      periodo_facturado_hasta:  ultimoDiaTxt,
      rubro:                    "Alquileres",
      rubro_grupo_contable:     "Ingresos",
      detalle: [{
        cantidad: 1,
        producto: {
  ...
  try {
    ...
    if (data.error === "N" && data.cae) {
      ...
      const registro = {
        ...
        nro:      data.comprobante_nro  || "",
        fecha:    fechaHoy,
        pdf_link: data.comprobante_pdf_url || ""
      };
      await push(ref(db, "factEmitidas"), registro);
      showToast(`Factura emitida ✓ CAE: ${data.cae}`, "success");
      renderFacturacion();
    } else {
      const errMsg = data.errores ? data.errores.join(" | ") : "Error desconocido";
      showToast(`Error ARCA: ${errMsg}`, "error");
      console.error("TusFacturas error:", data);
    }
  } catch (e) {
    showToast("Error de conexión con TusFacturasAPP", "error");
    console.error(e);
  } finally {
